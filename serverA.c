...
{ 
    ...
    servaddr.sin_port = htons(A_UDP_PORT); 
    ...
    if (tmp1 == 0)
    {
    sendto(sockfd, (const char *)m1 , 10, 0 , (const struct sockaddr *) &storage, (socklen_t)storagelen); 
    printf ("The Server A has found < 0 > match\n");
    }
    else
    {                                                        // Send DATA    
    sendto(sockfd, (const char *)m2 , 10, 0 , (const struct sockaddr *) &storage, (socklen_t)storagelen);
